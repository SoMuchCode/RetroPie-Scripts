...
if [ "$DBG" = "1" ]; then echo "rcos: running - $(date)" >> $LOGFILE; fi
...
then
	if [ "$DBG" = "1" ]; then echo "rcos: did nothing - exit 0 - enabled = $MODE" >> $LOGFILE; fi
...
then
	...
	then
		daemon="1"
		if [ "$DBG" = "1" ]; then echo "rcos: did nothing - daemon - exit 0 - enabled = $MODE" >> $LOGFILE; fi
		exit 0		# xboxdrv running as a daemon, just exit
	else
		daemon="0"	# not a daemon, we can continue
	fi
fi
# Currently no test for modes: 4/9, they are non-daemon mode
if [ "$DBG" = "1" ]; then echo "rcos: passed tests" >> $LOGFILE; fi
if [ "$DBG" = "1" ]; then echo "rcos: not a daemon" >> $LOGFILE; fi

# if we made it here, we are NOT a deamon and we are NOT disabled.
# we are in mode 1,2, or 4 or we would have exited by now

# We passed the tests so...
# it should be safe to use the 'joyhelp-nodaemon.cfg'
configfile=$XBDRDIR/joyhelp-nodaemon.cfg

# let us try and remove the xpad driver for kicks...
# IDK if this is a good idea, things seem to work fine without this happening...
# DON'T DO THIS!!! rmmod xpad Causes button mapping problems with my default config
# Hey it works now, I must have been doing something wrong before... yay :P
sudo rmmod xpad

# Load files created at system start based on: $XBDRDIR/joyhelp-config.cfg
launchPrefix=$(cat "$XBDRDIR/.xbd_prefix")
launchSuffix=$(cat "$XBDRDIR/.xbd_suffix")
p1_config=$(cat "$XBDRDIR/.xbd_p1config")
p2_config=$(cat "$XBDRDIR/.xbd_p2config")
p3_config=$(cat "$XBDRDIR/.xbd_p3config")
p4_config=$(cat "$XBDRDIR/.xbd_p4config")

# count controller config files so we know how many drivers to spawn...
controllers=4
if [ "$p4_config" = "" ]; then
...
if [ "$DBG" = "1" ]; then echo "rcos: Configured Controllers = $controllers" >> $LOGFILE; fi
# How did this happen??? 0 controllers connected
# Log but don't exit
if [ "$controllers" = "0" ] && [ "$DBG" = "1" ]; then echo "rcos: This doesn't sound right..." >> $LOGFILE; fi
...
####### END SECTION --------------------
if [ "$DBG" = "1" ]; then echo "rcos: Variables Initialized" >> $LOGFILE; fi
...
case $1 in
	...
	if [ "$tempFourWay" ]; then 
		tempcommand1=`echo $basicXBOX | sed "s/--deadzone/$fourway --deadzone/g" | sed "s|sleep..|sleep 1 \&\&|g"`	# This comand works on all controllers in config file
		tempcommand2=`echo $tempcommand1 | sed "s/--led 2/--led 2 $arcade_p1/g"`	# This command works on player 1
		tempcommand3=`echo $tempcommand2 | sed "s/--led 3/--led 3 $arcade_p2/g"`	# This command works on player 2
		tempcommand4=`echo $tempcommand3 | sed "s/--led 4/--led 4 $arcade_p3/g"`	# This command works on player 3
		tempcommand=`echo $tempcommand4 | sed "s/--led 5/--led 5 $arcade_p4/g"`		# This command works on player 4
		joycommand="$tempcommand"					# Execute combined joycommand
	elif [ "$tempFourWayFBA" ]; then 
		tempcommand1=`echo $basicXBOX | sed "s/--deadzone/$fourway --deadzone/g" | sed "s|sleep..|sleep 1 \&\&|g"`	# This comand works on all controllers in config file
		tempcommand2=`echo $tempcommand1 | sed "s/--led 2/--led 2 $arcade_p1/g"`	# This command works on player 1
		tempcommand3=`echo $tempcommand2 | sed "s/--led 3/--led 3 $arcade_p2/g"`	# This command works on player 2
		tempcommand4=`echo $tempcommand3 | sed "s/--led 4/--led 4 $arcade_p3/g"`	# This command works on player 3
		tempcommand=`echo $tempcommand4 | sed "s/--led 5/--led 5 $arcade_p4/g"`		# This command works on player 4
		joycommand="$tempcommand"					# Execute combined joycommand
	elif [ "$tempTrackball" ]; then 
		tempcommand1=`echo $basicXBOX | sed "s/--deadzone/$trackball --deadzone/g" | sed "s|sleep..|sleep 1 \&\&|g"`	# This comand works on all controllers in config file
		tempcommand2=`echo $tempcommand1 | sed "s/--led 2/--led 2 $arcade_p1/g"`	# This command works on player 1
		tempcommand3=`echo $tempcommand2 | sed "s/--led 3/--led 3 $arcade_p2/g"`	# This command works on player 2
		tempcommand4=`echo $tempcommand3 | sed "s/--led 4/--led 4 $arcade_p3/g"`	# This command works on player 3
		tempcommand=`echo $tempcommand4 | sed "s/--led 5/--led 5 $arcade_p4/g"`		# This command works on player 4
		...
		tempcommand1=`echo $basicXBOX | sed "s/--deadzone/$dial --deadzone/g" | sed "s|sleep..|sleep 1 \&\&|g"`	# This comand works on all controllers in config file
		tempcommand2=`echo $tempcommand1 | sed "s/--led 2/--led 2 $arcade_p1/g"`	# This command works on player 1
		tempcommand3=`echo $tempcommand2 | sed "s/--led 3/--led 3 $arcade_p2/g"`	# This command works on player 2
		tempcommand4=`echo $tempcommand3 | sed "s/--led 4/--led 4 $arcade_p3/g"`	# This command works on player 3
		tempcommand=`echo $tempcommand4 | sed "s/--led 5/--led 5 $arcade_p4/g"`		# This command works on player 4
		joycommand="$tempcommand"					# Execute combined joycommand
	elif [ "$tempAnalog" ]; then
		tempcommand1=`echo $basicXBOX | sed "s/--deadzone/$analog --deadzone/g" | sed "s|sleep..|sleep 1 \&\&|g"`	# This comand works on all controllers in config file
		tempcommand2=`echo $tempcommand1 | sed "s/--led 2/--led 2 $arcade_p1/g"`	# This command works on player 1
		tempcommand3=`echo $tempcommand2 | sed "s/--led 3/--led 3 $arcade_p2/g"`	# This command works on player 2
		tempcommand4=`echo $tempcommand3 | sed "s/--led 4/--led 4 $arcade_p3/g"`	# This command works on player 3
		tempcommand=`echo $tempcommand4 | sed "s/--led 5/--led 5 $arcade_p4/g"`		# This command works on player 4
		...
		tempcommand1=`echo $basicXBOX`
		tempcommand2=`echo $tempcommand1 | sed "s/--led 2/--led 2 $arcade_p1/g" | sed "s|sleep..|sleep 1 \&\&|g"`	# This command works on player 1
		tempcommand3=`echo $tempcommand2 | sed "s/--led 3/--led 3 $arcade_p2/g"`	# This command works on player 2
		tempcommand4=`echo $tempcommand3 | sed "s/--led 4/--led 4 $arcade_p3/g"`	# This command works on player 3
		tempcommand=`echo $tempcommand4 | sed "s/--led 5/--led 5 $arcade_p4/g"`		# This command works on player 4
		joycommand="$tempcommand"					# Execute combined joycommand
	...
	scummvm)
		tempcommand=`echo $basicXBOX | sed "s/--deadzone/$scummVM --deadzone/g" | sed "s|sleep..|sleep 1 \&\&|g"`
		joycommand="$tempcommand"
	;;

	amiga)
		tempcommand1=`echo $basicXBOX | sed "s|sleep..|sleep 1 \&\&|g"`
		tempcommand2=`echo $tempcommand1 | sed "s/--led 2/--led 2 $amigaplayer1/g"`
		tempcommand=`echo $tempcommand2 | sed "s/--led 3/--led 3 $amigaplayer2/g"`
		###tempcommand=`echo $basicXBOX | sed "s/--deadzone/$amiga --deadzone/g"`
		joycommand="$tempcommand"
	;;
	...
	intellivision)
		tempcommand=`echo $basicXBOX | sed "s|sleep..|sleep 1 \&\&|g"`
		joycommand="$tempcommand"
	;;
	...
	pc)
		tempcommand=`echo $basicXBOX | sed "s/--deadzone/$lim_axis --deadzone/g" | sed "s|sleep..|sleep 1 \&\&|g"`
		joycommand="$tempcommand"
	;;
	...
	quake)
		tempcommand=`echo $basicXBOX | sed "s/--deadzone/$invert_analog_y2 --deadzone/g" | sed "s|sleep..|sleep 1 \&\&|g"`
		joycommand="$tempcommand"
	;;
...
if [ "$DBG" = "1" ]; then echo "rcos: $joycommand" >> $LOGFILE; fi
eval $joycommand
